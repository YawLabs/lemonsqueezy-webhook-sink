 ...
 * run correctly and check all 11 files.
 *
 ...
 * there is no .github directory at all (CODEOWNERS sits at the repo root), so
 * no runner arbitrates formatting later. Whatever this script reports is the
 * ONLY lint signal that exists before `release.sh` publishes to npm.
 *
 ...
 * roughly 40ms to 2s to check this repo's 11 src/ files through the emulated
 ...
 * bug this replaced: $schema said 2.5.0 while the lockfile installed 2.5.1, so
 * the gate provisioned and ran 2.5.0 and "lint clean" was a claim about a
